from fastapi import APIRouter, HTTPException, Depends, status
from typing import List
from auth.auth import get_current_user
from database.database import get_expense_collection
from models.model import ExpenseCreate, ExpenseUpdate, ExpenseResponse
from bson import ObjectId
from datetime import datetime
from pymongo.collection import Collection

class ExpensesController:
    def __init__(self):
        self.router = APIRouter()

        # Register the routes
        self.router.post("/expenses", response_model=ExpenseResponse)(self.create_expense)
        self.router.get("/expenses", response_model=List[ExpenseResponse])(self.get_expenses)
        self.router.get("/expenses/{expenses_id}", response_model=ExpenseResponse)(self.get_expense)
        self.router.put("/expenses/{expenses_id}", response_model=ExpenseResponse)(self.update_expenses)
        self.router.delete("/expenses/{expenses_id}", response_model=dict)(self.delete_expenses)

    async def create_expense(
        self,
        expense: ExpenseCreate,
        token: str = Depends(get_current_user),
        expense_collection: Collection = Depends(get_expense_collection),
    ):
        expense_data = expense.dict()
        expense_data["owner"] = token
        expense_data["created_at"] = datetime.now()
        expense_data["last_modified"] = datetime.now()
        result = expense_collection.insert_one(expense_data)
        return ExpenseResponse(**expense_data, id=str(result.inserted_id))

    async def get_expenses(
        self,
        token: str = Depends(get_current_user),
        expense_collection: Collection = Depends(get_expense_collection),
    ):
        expenses = expense_collection.find({"owner": token})
        if not expenses:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No expenses found"
            )
        return [ExpenseResponse(**expense, id=str(expense["_id"])) for expense in expenses]

    async def get_expense(
        self,
        expenses_id: str,
        token: str = Depends(get_current_user),
        expense_collection: Collection = Depends(get_expense_collection),
    ):
        expense = expense_collection.find_one(
            {"_id": ObjectId(expenses_id), "owner": token}
        )
        if not expense:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found"
            )
        return ExpenseResponse(**expense, id=str(expense["_id"]))

    async def delete_expenses(
        self,
        expenses_id: str,
        token: str = Depends(get_current_user),
        expense_collection: Collection = Depends(get_expense_collection),
    ):
        expense = expense_collection.find_one(
            {"_id": ObjectId(expenses_id), "owner": token}
        )
        if not expense:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found"
            )
        if expense.get("status_done"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Expense is already done"
            )
        expense_collection.delete_one({"_id": ObjectId(expenses_id)})
        return {"Message": "Expense deleted successfully"}

    async def update_expenses(
        self,
        expenses_id: str,
        update_data: ExpenseUpdate,
        token: str = Depends(get_current_user),
        expense_collection: Collection = Depends(get_expense_collection),
    ):
        expense = expense_collection.find_one(
            {"_id": ObjectId(expenses_id), "owner": token}
        )
        if not expense:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found"
            )
        if expense.get("status_done"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Expense is already done"
            )
        update_dict = update_data.dict(exclude_unset=True)
        update_dict["last_modified"] = datetime.utcnow()
        expense_collection.update_one({"_id": ObjectId(expenses_id)}, {"$set": update_dict})
        updated_expense = expense_collection.find_one({"_id": ObjectId(expenses_id)})
        return ExpenseResponse(**updated_expense, id=str(updated_expense["_id"]))

# Instantiate the controller and export its router
expenses_controller = ExpensesController()
router = expenses_controller.router
