...
@router.post("/sell_stocks",response_model=SoldRecordResponse)
async def create_sold_record(sold: SoldRecordCreate, token: str=Depends(get_current_user),
    ...
    purchase = get_current_stocks_collection.find_one({"symbol":sold.symbol,"token":token})
    if not purchase:
    ...
    prev_net_profit = purchase.get("prev_net_profit",0.0)
    ...
    get_current_stocks_collection.update_one(
        {"_id":ObjectId(purchase["_id"])},
        {
            "$set": {
                ...
                "prev_net_profit": new_net_profit,
    ...
    sold_data["created_at"]=datetime.utcnow()
    sold_data["last_modified_at"]=datetime.utcnow
...
async def get_sold_records(token: str=Depends(get_current_user),
                             get_sold_collection: Collection = Depends(get_sold_collection)):
...
async def get_sold_record(sold_id:str,token: str=Depends(get_current_user),
                             get_sold_collection: Collection = Depends(get_sold_collection)):
    solds= get_sold_collection.find({"owner":token,"_id":ObjectId(sold_id)})
    if not solds:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No sold record found")
    return SoldRecordResponse(**solds, id=str(solds["_id"]))

@router.put("/sell_stocks/{sold_id}", response_model=SoldRecordResponse)
async def update_sold_record(sold_id:str,
                             update_data:SoldRecordCreate,
                             token: str=Depends(get_current_user),
                             get_current_stocks_collection: Collection = Depends(get_current_stocks_collection),
                             sold_collection: Collection = Depends(get_sold_collection)):
    
    existing_sold= sold_collection.find({"owner":token,"_id":ObjectId(sold_id)})
    if not existing_sold:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No sold record found")
    purchase =get_current_stocks_collection.find_one({"symbol":update_data.symbol, "owner":token})
    if not purchase:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No purchase record found")
    ...
    update_net_profit = reverted_net_profit + (update_data.price_per_unit_sold*update_data.quantity)
    get_current_stocks_collection.update_one(
        {"_id":ObjectId(purchase["_id"])},
        {
            "$set":{
...
async def delete_sold_record(sold_id:str,
                             token: str=Depends(get_current_user),
                             get_current_stocks_collection: Collection = Depends(get_current_stocks_collection),
                             sold_collection: Collection = Depends(get_sold_collection)):
    ...
    if not existing_sold:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No sold record found")
    purchase =get_current_stocks_collection.find_one({"symbol":existing_sold["symbol"], "owner":token})
    if not purchase:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No purchase record found")
    ...
    if not result:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete sold record")
